...
class RGBProcessor:
    '''
    Process RGB frames as inputs of Fall Detector model
    '''
    def __init__(self, dim=128):
        # Initialize RGB Processor parameters
        self.dim = dim
        
    def process(self, frame, save_batch=True):
        # Resize the RGB frame to the desired dimensions
        frame_resized = cv2.resize(frame, (self.dim, self.dim),
                                   interpolation=cv2.INTER_AREA)
        
        if save_batch:
            return frame_resized
        else:
            # Encode the resized frame as PNG
            flag, encode_img = cv2.imencode('.png', frame_resized)
            if flag:
